...
function Ourproduct() {
    const [emblaRef,emblaApi]=useEmblaCarousel({loop:true,align:"start"},[AutoPlay({stopOnMouseEnter:true})])
    useEffect(() => {
        if (!emblaApi) return;
    ...
    <div>
      ...
      <div className='relative w-full h-full'>
        ...
        className='overflow-hidden overscroll-contain'>
            <div className='flex '>
                {Array.from({length:Math.ceil(secProduct.length/2)}).map((_,index)=> (
                    <div key={index} 
                    className='flex-shrink-0 max-w-[325px] w-full'
                    >
