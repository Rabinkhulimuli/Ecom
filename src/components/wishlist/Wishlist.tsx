    ...
    <div className=" space-y-10 sm:space-y-15">
      <div className="flex justify-between items-center">
        <p className="text-lg md:text-xl flex w-fit items-center">
          <span>
            Wishlist <span className="text-sm">(4)</span>
          </span>
        </p>
        <p className="border text-sm md:text-lg w-fit px-4 py-2 md:px-12 md:py-4 rounded-sm">
          Move All To Bag
        </p>
      </div>
      <div className=" grid   lg:gap-y-6 gap-1 sm:gap-2 grid-cols-2 sm:grid-cols-3 justify-between lg:grid-cols-4">
        {secProduct.map((eh) => (
          ...
          >
            ...
            <div
              className={` absolute inset-0 m-x-auto flex   h-fit ${eh.discount === 0 ? "justify-end" : "justify-between"}  top-1 sm:top-3  px-1 py-0.5 md:px-3 md:py-1 `}
            >
              <p
                className={`discount-text ${eh.discount === 0 ? "hidden" : "block"}`}
              >
