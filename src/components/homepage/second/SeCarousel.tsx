...
const SeCarousel = () => {
  ...
  if (products.length === 0) return <div className="h-64 flex items-center justify-center">No products available</div>;
...
const ProductCard = React.memo(({ product }: { product: allProductResponse }) => {
      ...
      <div className="relative flex py-10 min-w-67 h-62 overflow-hidden justify-center bg-gray-100">
        <div className="max-w-43 max-h-38">
          <Image
            ...
            src={imgError ? "/nav/defaultimage.jpg" : product.images[0]?.path}
            width={1024}
            height={720}
            alt={product.name}
            priority
